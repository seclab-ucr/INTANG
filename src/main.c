...
int opt_protect_https_protocol = 1;
...
int add_iptables_rules()
{
    ...
    system(cmd);
    // Dulicate rules may be generated by the following code. But it doesn't matter, unless it does...
    ...
    if (opt_protect_https_protocol) {
        // monitor incoming RST
        sprintf(cmd, "iptables -A INPUT -p tcp --sport 443 --tcp-flags RST RST -j NFQUEUE --queue-num %d", NF_QUEUE_NUM);
        system(cmd);
        // monitor outgoing SYN
        sprintf(cmd, "iptables -A OUTPUT -p tcp --dport 443 --tcp-flags SYN,ACK SYN -m mark ! --mark %d -j NFQUEUE --queue-num %d", MARK, NF_QUEUE_NUM);
        system(cmd);
        // monitor incoming SYN-ACK
        sprintf(cmd, "iptables -A INPUT -p tcp --sport 443 --tcp-flags SYN,ACK SYN,ACK -j NFQUEUE --queue-num %d", NF_QUEUE_NUM);
        system(cmd);
        // intercept outgoing ACK
        sprintf(cmd, "iptables -A OUTPUT -p tcp --dport 443 --tcp-flags SYN,ACK,RST ACK -m mark ! --mark %d -m length --length 0:80 -j NFQUEUE --queue-num %d", MARK, NF_QUEUE_NUM);
        system(cmd);
        // intercept outgoing HTTPS ClientHello message
        sprintf(cmd, "iptables -A OUTPUT -p tcp --dport 443 --tcp-flags SYN,ACK,RST ACK -m mark ! --mark %d -m u32 --u32 '0>>22&0x3C@ 12>>26&0x3C@ 0&0xFFFF0000=0x16030000' -j NFQUEUE --queue-num %d", MARK, NF_QUEUE_NUM);
        system(cmd);
        // intercept incoming HTTPS ServerHello message
        sprintf(cmd, "iptables -A INPUT -p tcp --sport 443 --tcp-flags SYN,ACK,RST ACK -m u32 --u32 '0>>22&0x3C@ 12>>26&0x3C@ 0&0xFFFF0000=0x16030000' -j NFQUEUE --queue-num %d", NF_QUEUE_NUM);
        system(cmd);
    }
...
int remove_iptables_rules()
{
    ...
    if (opt_protect_https_protocol) {
        // monitor incoming RST
        sprintf(cmd, "iptables -D INPUT -p tcp --sport 443 --tcp-flags RST RST -j NFQUEUE --queue-num %d", NF_QUEUE_NUM);
        system(cmd);
        // monitor outgoing SYN
        sprintf(cmd, "iptables -D OUTPUT -p tcp --dport 443 --tcp-flags SYN,ACK SYN -m mark ! --mark %d -j NFQUEUE --queue-num %d", MARK, NF_QUEUE_NUM);
        system(cmd);
        // monitor incoming SYN-ACK
        sprintf(cmd, "iptables -D INPUT -p tcp --sport 443 --tcp-flags SYN,ACK SYN,ACK -j NFQUEUE --queue-num %d", NF_QUEUE_NUM);
        system(cmd);
        // intercept outgoing ACK
        sprintf(cmd, "iptables -D OUTPUT -p tcp --dport 443 --tcp-flags SYN,ACK,RST ACK -m mark ! --mark %d -m length --length 0:80 -j NFQUEUE --queue-num %d", MARK, NF_QUEUE_NUM);
        system(cmd);
        // intercept outgoing HTTPS ClientHello message
        sprintf(cmd, "iptables -D OUTPUT -p tcp --dport 443 --tcp-flags SYN,ACK,RST ACK -m mark ! --mark %d -m u32 --u32 '0>>22&0x3C@ 12>>26&0x3C@ 0&0xFFFF0000=0x16030000' -j NFQUEUE --queue-num %d", MARK, NF_QUEUE_NUM);
        system(cmd);
        // intercept incoming HTTPS ServerHello message
        sprintf(cmd, "iptables -D INPUT -p tcp --sport 443 --tcp-flags SYN,ACK,RST ACK -m u32 --u32 '0>>22&0x3C@ 12>>26&0x3C@ 0&0xFFFF0000=0x16030000' -j NFQUEUE --queue-num %d", NF_QUEUE_NUM);
        system(cmd);
    }
    if (opt_protect_dns_protocol) {
        // monitor incoming RST
 ...
 * inout: 0, incoming packet; 1, outgoing packet.
 */
...
int process_tcp_packet(struct mypacket *packet, char inout)
{
    ...
    if (inout == 1) {
        ...
        if (tcphdr->th_flags == TCP_SYN) {
            ...
            // Processing outgoing data packet.
            ...
            else if (dport == 443 && is_https_client_hello(payload)) {
                log_debug("Got a ClientHello message.");
                // it's an HTTPS ClientHello message. 
                // Keep in mind that Tor or other protocols can also uses port 443!
                // Use the same logic as Tor and OpenVPN for now.
                int sid = get_sid(&fourtp);
                if (sid >= 0 && g_strats[sid].process_request) {
                    ret = g_strats[sid].process_request(packet);
                    if (ret) {
        ...
        if (tcphdr->th_flags == (TCP_SYN | TCP_ACK)) {
            ...
            else if (sport == 443 && is_https_server_hello(payload)) {
                // it's an HTTPS ServerHello message. 
                // Keep in mind that Tor or other protocols can also uses port 443!
                log_debug("Got a ServerHello message.");
            }
